...
`Python` · `LangGraph` · `LangChain` · `Multi-Agent Systems` · `Agent Orchestration` · `Agentic Workflows` · `Human-in-the-Loop` · `Conditional Routing` · `ReAct` · `CrewAI`

### LLMs & RAG

`RAG` · `pgvector` · `Semantic Search` · `Vector Embeddings` · `Prompt Engineering` · `Tool Calling` · `Structured Extraction` · `Output Guardrails` · `LLM Evaluation` · `Context Management`

### Backend & APIs

`Python` · `FastAPI` · `REST APIs` · `Supabase` · `PostgreSQL` · `Edge Functions` · `Webhooks` · `Serverless Architecture`

### Observability & Evaluation

`LangSmith` · `LLM Tracing` · `Pipeline Monitoring` · `Root Cause Analysis` · `Agent Failure Diagnosis` · `QA Evaluation`

### Frontend & Deployment

`Next.js` · `TypeScript` · `Vercel` · `Supabase Realtime` · `GitHub API` · `PyGitHub` · `Zod`

### Automation & Developer Tools

`n8n` · `API Orchestration` · `Git` · `GitHub` · `Cursor`
